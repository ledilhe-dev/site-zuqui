...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if(body.action==="managerial_sync"){
      ...
      return json({ok:true,abc:abc.length,itens_obrigatorios:mandatory.length});
    }
    ...
    if(body.action==="mandatory_dashboard"){
      ...
      const matchesGroup=(x:any)=>{if(!groupTerms.length)return true;const child=productGroups.get(String(x.id_componente)),parentId=String(x.id_agrupamento_pai??""),parentName=String(x.agrupamento_pai||"").toLocaleLowerCase("pt-BR");return groupTerms.some((term:string)=>term===parentId||term===parentName||term===child?.id||term===child?.nome)};
      const filtered=rows.filter(x=>{const dt=`${x.data}T${String(x.hora||"00:00:00").slice(0,8)}`;return dt>=start&&dt<end&&matchesGroup(x)&&(!parentText||String(x.produto_pai||"").toLocaleLowerCase("pt-BR").includes(parentText));}),parents=new Map<string,any[]>();for(const x of filtered){const k=String(x.id_pai),list=parents.get(k)||[];list.push(x);parents.set(k,list);}const flavors=new Map<string,any>(),combos=new Map<string,any>(),parentAgg=new Map<string,any>(),channels=new Map<string,any>(),details:any[]=[];let whole=0,half=0,multiple=0,revenue=0;for(const list of parents.values()){const p=list[0],n=list.length,type=n===1?"inteira":n===2?"meia":"multipla",value=Number(p.valor_item||0);if(type==="inteira")whole++;else if(type==="meia")half++;else multiple++;revenue+=value;const sorted=[...list].sort((a,b)=>Number(a.id_componente)-Number(b.id_componente)),combo=sorted.map(x=>x.componente).join(" + ");if(n>=2){const c=combos.get(combo)||{combinacao:combo,quantidade:0};c.quantidade++;combos.set(combo,c);}for(const x of list){const k=String(x.id_componente),f=flavors.get(k)||{id:x.id_componente,sabor:x.componente,inteiras:0,participacoes_meia:0,participacoes_multiplas:0,equivalente:0,faturamento_atribuido:0};if(type==="inteira")f.inteiras++;else if(type==="meia")f.participacoes_meia++;else f.participacoes_multiplas++;f.equivalente+=1/n;f.faturamento_atribuido+=value/n;flavors.set(k,f);}const pa=parentAgg.get(String(p.id_produto_pai))||{id:p.id_produto_pai,produto:p.produto_pai,agrupamento:p.agrupamento_pai,total:0,inteiras:0,meias:0,multiplas:0,faturamento:0};pa.total++;pa[type==="inteira"?"inteiras":type==="meia"?"meias":"multiplas"]++;pa.faturamento+=value;parentAgg.set(String(p.id_produto_pai),pa);const ch=channels.get(p.origem)||{canal:p.origem,total:0,inteiras:0,meias:0,multiplas:0,faturamento:0};ch.total++;ch[type==="inteira"?"inteiras":type==="meia"?"meias":"multiplas"]++;ch.faturamento+=value;channels.set(p.origem,ch);if(body.detalhes)details.push({...p,tipo:type,sabores:combo});}const total=parents.size;return json({resumo:{total,inteiras:whole,meias:half,multiplas:multiple,percentual_inteiras:total?whole/total*100:0,percentual_meias:total?half/total*100:0,faturamento:revenue,ticket_medio:total?revenue/total:0,sabores:flavors.size},sabores:[...flavors.values()].sort((a,b)=>b.equivalente-a.equivalente),combinacoes:[...combos.values()].sort((a,b)=>b.quantidade-a.quantidade),pais:[...parentAgg.values()].sort((a,b)=>b.total-a.total),canais:[...channels.values()].sort((a,b)=>b.total-a.total),detalhes:details,rastreamento:{cache:rows.length,filtrado:filtered.length,pais:parents.size},origem_consulta:"cache_gerencial_remoto"});
    }
