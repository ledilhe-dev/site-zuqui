...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if(body.action==="pizza_mandatory_sync_v1"){
      const integration=await integrationForToken(admin,validateToken(body.token),body,true),filial=Number(body.id_filial),kind=String(body.kind||"data"),syncedAt=new Date().toISOString();
      if(filial!==Number(integration.raffinato_filial_id))throw new Error("Filial enviada pelo conector diverge do vinculo da loja.");
      const base={empresa_id:integration.empresa_id,loja_id:integration.loja_id,connector_instance_id:integration.connector_instance_id,connection_profile_id:integration.connection_profile_id,id_filial:filial,dataset_version:"pizza-v1",sincronizado_em:syncedAt};
      if(kind==="metadata"){
        const items=Array.isArray(body.agrupamentos)?body.agrupamentos.slice(0,10000):[],del=await admin.from("raffinato_pizza_mandatory_metadata_v1").delete().eq("empresa_id",integration.empresa_id).eq("loja_id",integration.loja_id).eq("id_filial",filial).eq("dataset_version","pizza-v1");if(del.error)throw del.error;
        for(let o=0;o<items.length;o+=1000){const rows=items.slice(o,o+1000).map((x:any)=>({...base,id_agrupamento:Number(x.id),arvore:String(x.arvore||"").slice(0,500)||null,nome:String(x.nome||"").trim().slice(0,300)})).filter((x:any)=>x.nome),result=await admin.from("raffinato_pizza_mandatory_metadata_v1").insert(rows);if(result.error)throw result.error;}const touch=await admin.from("raffinato_integracoes").update({ultima_sincronizacao_em:syncedAt,ultimo_erro:null}).eq("id",integration.id);if(touch.error)throw touch.error;return json({ok:true,metadata_count:items.length,resolved_raffinato_filial_id:filial});
      }
      const inicio=validateDate(body.inicio,"inicio"),fim=validateDate(body.fim,"fim"),items=Array.isArray(body.items)?body.items.slice(0,100000):[],del=await admin.from("raffinato_pizza_mandatory_data_v1").delete().eq("empresa_id",integration.empresa_id).eq("loja_id",integration.loja_id).eq("id_filial",filial).eq("dataset_version","pizza-v1").gte("data",inicio).lte("data",fim);if(del.error)throw del.error;
      for(let o=0;o<items.length;o+=1000){const rows=items.slice(o,o+1000).map((x:any)=>({...base,data:validateDate(x.data,"data"),hora:String(x.hora||"00:00:00").slice(0,8),id_venda:Number(x.id_venda),id_item:Number(x.id_item),id_pai:Number(x.id_pai),id_produto_pai:x.id_produto_pai==null?null:Number(x.id_produto_pai),produto_pai:String(x.produto_pai||"").slice(0,300),id_agrupamento_pai:x.id_agrupamento_pai==null?null:Number(x.id_agrupamento_pai),agrupamento_pai:String(x.agrupamento_pai||"").slice(0,300),quantidade_produto_principal:Number(x.quantidade_produto_principal||0),origem:String(x.modulo_venda||x.origem||"SEM_ORIGEM"),modulo_venda:String(x.modulo_venda||x.origem||"SEM_ORIGEM"),canal_venda:String(x.canal_venda||"NAO_IDENTIFICADO").slice(0,200),situacao_venda:String(x.situacao_venda||"NAO_IDENTIFICADA"),origem_codigo:x.origem_codigo==null?null:Number(x.origem_codigo),vinculos_modulo:Number(x.vinculos_modulo||0),valor_item:Number(x.valor_item||0),id_grupo_obrigatorio:Number(x.id_grupo_obrigatorio),grupo_obrigatorio:String(x.grupo_obrigatorio||"").slice(0,300),quantidade_maxima:x.quantidade_maxima==null?null:Number(x.quantidade_maxima),quantidade_minima:x.quantidade_minima==null?null:Number(x.quantidade_minima),id_componente:x.id_componente==null?null:Number(x.id_componente),componente:String(x.componente||"").slice(0,300),quantidade_componente:Number(x.quantidade_componente||0),valor_unitario_componente:Number(x.valor_unitario_componente||0),valor_componente:Number(x.valor_componente||0),cancelado:x.cancelado==null?null:Boolean(x.cancelado),retornou_estoque:x.retornou_estoque==null?null:Boolean(x.retornou_estoque),retorno_estoque_original:x.retorno_estoque_original==null?null:String(x.retorno_estoque_original).slice(0,1)})),result=await admin.from("raffinato_pizza_mandatory_data_v1").upsert(rows,{onConflict:"empresa_id,loja_id,id_filial,dataset_version,id_venda,id_pai,id_grupo_obrigatorio,id_item"});if(result.error)throw result.error;}return json({ok:true,quantidade:items.length,resolved_raffinato_filial_id:filial});
    }
    ...
    if(mappedReportActions.has(String(body.action||""))){
      ...
      if(!mapping||mapping.raffinato_filial_id==null||!mapping.connection_profile_id)throw new Error(String(body.action||"").startsWith("pizza_mandatory_")?"Esta loja não possui uma filial Raffinato vinculada.":"Esta loja ainda nao possui uma filial Raffinato vinculada.");
    ...
    if(body.action==="pizza_mandatory_metadata_v1"){
      ...
      return json({resolved_raffinato_filial_id:filial,connection_profile_id:body.connection_profile_id,metadata_count:rows.length,agrupamentos:rows.map(x=>({id:x.id_agrupamento,arvore:x.arvore,nome:x.nome})),sincronizado_em:rows.map(x=>x.sincronizado_em).sort().at(-1)||null});
    }

    if(body.action==="pizza_mandatory_report_v1"){
...
const groups=new Set((Array.isArray(body.agrupamentos)?body.agrupamentos:[]).map(Number)),product=String(body.produto||"").trim().toLocaleLowerCase("pt-BR"),filtered=rows.filter(x=>{const stamp=`${x.data}T${String(x.hora||"00:00:00").slice(0,8)}`;return stamp>=start&&stamp<end&&(!groups.size||groups.has(Number(x.id_agrupamento_pai)))&&(!product||String(x.id_produto_pai)===product||String(x.produto_pai||"").toLocaleLowerCase("pt-BR").includes(product));});
    ...
    if (body.action === "annual_comparison") {
      ...
      const first=Math.max(1900,Number(body.ano_inicial||new Date().getFullYear()-1)),last=Math.min(2100,Number(body.ano_final||new Date().getFullYear()));if(last<first)throw new Error("Intervalo de anos invalido.");const annualSnapshot=(await loadAnnualCache()).filter(x=>Number(x.ano)>=first&&Number(x.ano)<=last);
      ...
      const docs=await loadAnnual("raffinato_documentos_faturados_cache","id_documento_fiscal,data,hora,eh_contingencia,modulo_venda,valor_pagamento"),items=await loadAnnual("raffinato_itens_faturados_cache","id_documento_fiscal,data,hora,codigo,produto,id_agrupamento,agrupamento,quantidade,total_faturado"),open=await loadAnnual("raffinato_delivery_aberto_cache","id_tele_entrega,data,hora,valor,cancelado,finalizado,id_documento_fiscal");
      const docMap=new Map<string,any>();for(const d of docs){const id=String(d.id_documento_fiscal),x=docMap.get(id)||{id,data:d.data,hora:d.hora||"00:00:00",eh_contingencia:Boolean(d.eh_contingencia),modulo_venda:d.modulo_venda||"VENDA_RAPIDA",faturamento:0};x.faturamento+=Number(d.valor_pagamento||0);docMap.set(id,x);}
      const productText=String(body.produto||"").trim().toLocaleLowerCase("pt-BR"),groupIds=[...new Set((Array.isArray(body.id_agrupamentos)?body.id_agrupamentos:(body.id_agrupamento==null?[]:[body.id_agrupamento])).map((x:any)=>String(x)).filter(Boolean))],groupSet=new Set(groupIds),eligibleByProduct=new Set(items.filter(x=>(!groupSet.size||groupSet.has(String(x.id_agrupamento)))&&(!productText||String(x.codigo)===productText||String(x.produto||"").toLocaleLowerCase("pt-BR").includes(productText))).map(x=>String(x.id_documento_fiscal))),hasItemFilter=!!(groupSet.size||productText),moduleIds=[...new Set((Array.isArray(body.modulos_venda)?body.modulos_venda:(body.modulo_venda?[body.modulo_venda]:[])).map((x:any)=>String(x)).filter(Boolean))],moduleSet=new Set(moduleIds);
      const issuance=String(body.emissao||"TODOS");if(!["TODOS","CONTINGENCIA","SEM_CONTINGENCIA"].includes(issuance))throw new Error("Filtro de emissao fiscal invalido.");const weekday=body.dia_semana==null?null:Number(body.dia_semana),hour=body.hora==null?null:Number(body.hora),dayOf=(d:string)=>new Date(`${d}T12:00:00Z`).getUTCDay(),eligibleDocs=[...docMap.values()].filter(x=>(issuance==="TODOS"||(issuance==="CONTINGENCIA"?x.eh_contingencia:!x.eh_contingencia))&&(!moduleSet.size||moduleSet.has(String(x.modulo_venda)))&&(!hasItemFilter||eligibleByProduct.has(x.id))&&(weekday==null||dayOf(x.data)===weekday)&&(hour==null||Number(String(x.hora).slice(0,2))===hour)),eligibleIds=new Set(eligibleDocs.map(x=>x.id)),eligibleItems=items.filter(x=>eligibleIds.has(String(x.id_documento_fiscal))&&(!groupSet.size||groupSet.has(String(x.id_agrupamento)))&&(!productText||String(x.codigo)===productText||String(x.produto||"").toLocaleLowerCase("pt-BR").includes(productText)));
      const monthMap=new Map<string,any>(),moduleMonthMap=new Map<string,any>();for(const d of eligibleDocs){const year=Number(String(d.data).slice(0,4)),month=Number(String(d.data).slice(5,7)),key=`${year}-${month}`,x=monthMap.get(key)||{ano:year,mes:month,faturamento:0,vendas:0,quantidade:0,faturamento_contingencia:0,faturamento_sem_contingencia:0,vendas_contingencia:0,vendas_sem_contingencia:0};x.faturamento+=d.faturamento;x.vendas++;if(d.eh_contingencia){x.faturamento_contingencia+=d.faturamento;x.vendas_contingencia++}else{x.faturamento_sem_contingencia+=d.faturamento;x.vendas_sem_contingencia++}monthMap.set(key,x);const mk=`${d.modulo_venda}-${year}-${month}`,mx=moduleMonthMap.get(mk)||{modulo_venda:d.modulo_venda,ano:year,mes:month,faturamento:0};mx.faturamento+=d.faturamento;moduleMonthMap.set(mk,mx);}
      ...
      if(issuance!=="CONTINGENCIA"&&!hasItemFilter&&(!moduleSet.size||moduleSet.has("DELIVERY")))for(const d of open.filter(x=>!x.cancelado&&!x.finalizado&&!x.id_documento_fiscal)){const year=Number(String(d.data).slice(0,4)),month=Number(String(d.data).slice(5,7)),value=Number(d.valor||0),key=`${year}-${month}`,x=monthMap.get(key)||{ano:year,mes:month,faturamento:0,vendas:0,quantidade:0,faturamento_contingencia:0,faturamento_sem_contingencia:0,vendas_contingencia:0,vendas_sem_contingencia:0};x.faturamento+=value;x.faturamento_sem_contingencia+=value;x.vendas++;x.vendas_sem_contingencia++;monthMap.set(key,x);const mk=`DELIVERY-${year}-${month}`,mx=moduleMonthMap.get(mk)||{modulo_venda:"DELIVERY",ano:year,mes:month,faturamento:0};mx.faturamento+=value;moduleMonthMap.set(mk,mx);}
      if(issuance==="TODOS"&&!hasItemFilter){const source=moduleSet.size?annualSnapshot.filter(x=>moduleSet.has(String(x.modulo_venda))):annualSnapshot.filter(x=>String(x.modulo_venda)==="TODOS"),historicalMap=new Map<string,any>();for(const h of source){const key=`${h.ano}-${h.mes}`,sum=historicalMap.get(key)||{ano:Number(h.ano),mes:Number(h.mes),faturamento:0,vendas:0,quantidade:0};sum.faturamento+=Number(h.faturamento||0);sum.vendas+=Number(h.vendas||0);sum.quantidade+=Number(h.quantidade||0);historicalMap.set(key,sum)}for(const [key,h] of historicalMap){const fresh=monthMap.get(key)||{ano:h.ano,mes:h.mes,faturamento_contingencia:0,faturamento_sem_contingencia:0,vendas_contingencia:0,vendas_sem_contingencia:0};fresh.faturamento=h.faturamento;fresh.vendas=h.vendas;fresh.quantidade=h.quantidade;monthMap.set(key,fresh)}}
...
async function integrationForToken(admin: any, token: string, body:any={}, requireMapping=false) {
