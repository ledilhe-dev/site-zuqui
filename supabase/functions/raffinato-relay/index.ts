...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === "annual_comparison") {
      ...
      const first=Math.max(2000,Number(body.ano_inicial||new Date().getFullYear()-1)),last=Math.min(2100,Number(body.ano_final||new Date().getFullYear()));if(last<first||last-first>7)throw new Error("Intervalo de anos invalido.");
      ...
      const docs=await loadAnnual("raffinato_documentos_faturados_cache","id_documento_fiscal,data,modulo_venda,valor_pagamento"),items=await loadAnnual("raffinato_itens_faturados_cache","id_documento_fiscal,data,codigo,produto,id_agrupamento,agrupamento,quantidade,total_faturado"),open=await loadAnnual("raffinato_delivery_aberto_cache","id_tele_entrega,data,valor,cancelado,finalizado,id_documento_fiscal");
      const docMap=new Map<string,any>();for(const d of docs){const id=String(d.id_documento_fiscal),x=docMap.get(id)||{id,data:d.data,modulo_venda:d.modulo_venda||"VENDA_RAPIDA",faturamento:0};x.faturamento+=Number(d.valor_pagamento||0);docMap.set(id,x);}
      ...
      const eligibleDocs=[...docMap.values()].filter(x=>(!module||x.modulo_venda===module)&&(!hasItemFilter||eligibleByProduct.has(x.id))),eligibleIds=new Set(eligibleDocs.map(x=>x.id)),eligibleItems=items.filter(x=>eligibleIds.has(String(x.id_documento_fiscal))&&(!group||String(x.id_agrupamento)===group)&&(!productText||String(x.codigo)===productText||String(x.produto||"").toLocaleLowerCase("pt-BR").includes(productText)));
      ...
      const selectedYear=Number(body.ano||last),selectedMonth=Number(body.mes||0),periodIds=new Set(eligibleDocs.filter(x=>Number(String(x.data).slice(0,4))===selectedYear&&(!selectedMonth||Number(String(x.data).slice(5,7))===selectedMonth)).map(x=>x.id)),periodItems=eligibleItems.filter(x=>periodIds.has(String(x.id_documento_fiscal))),productMap=new Map<string,any>(),groupMap=new Map<string,any>(),dayMap=new Map<string,any>();for(const x of periodItems){const id=String(x.codigo),p=productMap.get(id)||{id,nome:x.produto||id,faturamento:0,quantidade:0};p.faturamento+=Number(x.total_faturado||0);p.quantidade+=Number(x.quantidade||0);productMap.set(id,p);const gid=String(x.id_agrupamento||"SEM_GRUPO"),g=groupMap.get(gid)||{id:gid,nome:x.agrupamento||"Sem agrupamento",faturamento:0};g.faturamento+=Number(x.total_faturado||0);groupMap.set(gid,g);}for(const d of eligibleDocs.filter(x=>periodIds.has(x.id))){const day=String(d.data),x=dayMap.get(day)||{data:day,faturamento:0,vendas:0};x.faturamento+=d.faturamento;x.vendas++;dayMap.set(day,x);}const limit=Math.min(20,Math.max(10,Number(body.limite||10))),allProducts=[...productMap.values()];return json({schema_version:1,produtos:[...allProducts].sort((a,b)=>b.faturamento-a.faturamento).slice(0,limit),produtos_quantidade:[...allProducts].sort((a,b)=>b.quantidade-a.quantidade).slice(0,limit),agrupamentos:[...groupMap.values()].sort((a,b)=>b.faturamento-a.faturamento).slice(0,15),dias:[...dayMap.values()].sort((a,b)=>a.data.localeCompare(b.data))});
    }
