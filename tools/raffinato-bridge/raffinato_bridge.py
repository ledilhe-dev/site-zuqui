...
SET NOCOUNT ON;
SELECT VI.Id id_item,V.Id id_venda,CONVERT(date,V.Data) data,CONVERT(varchar(8),CAST(V.Hora AS time),108) hora,
 CASE WHEN ({link_count})>1 THEN 'ORIGEM_INCONSISTENTE'
      WHEN EXISTS(SELECT 1 FROM dbo.VendaTeleEntrega T WITH(NOLOCK) WHERE T.IdVenda=V.Id) THEN 'DELIVERY'
      WHEN EXISTS(SELECT 1 FROM dbo.VendaCartaoConsumo C WITH(NOLOCK) WHERE C.IdVenda=V.Id) THEN 'CARTAO_CONSUMO'
      WHEN EXISTS(SELECT 1 FROM dbo.VendaMesa M WITH(NOLOCK) WHERE M.IdVenda=V.Id) THEN 'MESA'
      {mobilidade_case}
      WHEN V.Origem=1 THEN 'VENDA_RAPIDA'
      WHEN V.Origem IS NULL OR V.Origem=0 THEN 'SEM_ORIGEM'
      ELSE 'OUTRO_MODULO' END modulo_venda,
 V.Origem origem_codigo,({link_count}) vinculos_modulo,
 {canal_select} canal_venda,
 {situacao_select} situacao_venda,
 PAI.Id id_pai,PAI.IdProduto id_produto_pai,PP.Nome produto_pai,PP.IdAgrupamento id_agrupamento_pai,A.Nome agrupamento_pai,
 ...
 CAST(ISNULL(VI.ValorTotal,0) AS decimal(19,4)) valor_componente,
 CAST(CASE WHEN VI.IdStatusItem=2 OR PAI.IdStatusItem=2 THEN 1 ELSE 0 END AS bit) cancelado,
 {stock_return_select}
FROM dbo.Venda V WITH(NOLOCK)
INNER JOIN dbo.VendaItem VI WITH(NOLOCK) ON VI.IdVenda=V.Id
 AND VI.IdAgrupamentoItemObrigatorio IS NOT NULL AND VI.IdItemPai IS NOT NULL
INNER JOIN dbo.VendaItem PAI WITH(NOLOCK) ON PAI.Id=VI.IdItemPai AND PAI.IdVenda=V.Id
...
LEFT JOIN dbo.Produto PI WITH(NOLOCK) ON PI.Id=VI.IdProduto
WHERE V.IdFilial=? AND V.Data>=? AND V.Data<?
 AND DATEADD(SECOND,DATEDIFF(SECOND,CAST('00:00:00' AS time),CAST(V.Hora AS time)),CAST(CONVERT(date,V.Data) AS datetime2))>=?
 AND DATEADD(SECOND,DATEDIFF(SECOND,CAST('00:00:00' AS time),CAST(V.Hora AS time)),CAST(CONVERT(date,V.Data) AS datetime2))<?
 {dynamic_filters}
ORDER BY V.Data,V.Hora,PAI.Id,VI.Id;
"""
...
def query_pizza_mandatory_rows_v1(config:dict[str,Any],start:datetime,end:datetime,filial:int) -> list[dict[str,Any]]:
    with pyodbc.connect(connection_string(config),timeout=8) as connection:
        ...
        link_count=("(CASE WHEN EXISTS(SELECT 1 FROM dbo.VendaTeleEntrega T WITH(NOLOCK) WHERE T.IdVenda=V.Id) THEN 1 ELSE 0 END + "
                    "CASE WHEN EXISTS(SELECT 1 FROM dbo.VendaCartaoConsumo C WITH(NOLOCK) WHERE C.IdVenda=V.Id) THEN 1 ELSE 0 END + "
                    "CASE WHEN EXISTS(SELECT 1 FROM dbo.VendaMesa M WITH(NOLOCK) WHERE M.IdVenda=V.Id) THEN 1 ELSE 0 END + "
                    f"CASE WHEN {mobility_exists} THEN 1 ELSE 0 END)")
        canal_select=("CASE WHEN EXISTS(SELECT 1 FROM dbo.VendaTeleEntrega T WITH(NOLOCK) WHERE T.IdVenda=V.Id) THEN "
          "COALESCE((SELECT TOP 1 NULLIF(OI.Descricao,'') FROM dbo.VendaTeleEntrega VT WITH(NOLOCK) "
          "JOIN dbo.TeleEntrega TE WITH(NOLOCK) ON TE.Id=VT.IdTeleEntrega LEFT JOIN dbo.OrigemIntegracao OI WITH(NOLOCK) ON OI.Id=TE.IdOrigemIntegracao "
          "WHERE VT.IdVenda=V.Id),'CANAL_NAO_IDENTIFICADO') ELSE 'LOJA_MANUAL' END") if has_channel else (
          "CASE WHEN EXISTS(SELECT 1 FROM dbo.VendaTeleEntrega T WITH(NOLOCK) WHERE T.IdVenda=V.Id) THEN 'CANAL_NAO_IDENTIFICADO' ELSE 'LOJA_MANUAL' END")
        situacao_select=("CASE WHEN EXISTS(SELECT 1 FROM dbo.VendaTeleEntrega T WITH(NOLOCK) WHERE T.IdVenda=V.Id AND ISNULL(T.Aberto,0)=1) "
          "OR EXISTS(SELECT 1 FROM dbo.VendaCartaoConsumo C WITH(NOLOCK) WHERE C.IdVenda=V.Id AND ISNULL(C.Aberto,0)=1) "
          "OR EXISTS(SELECT 1 FROM dbo.VendaMesa M WITH(NOLOCK) WHERE M.IdVenda=V.Id AND ISNULL(M.Aberto,0)=1) THEN 'EM_ABERTO' "
...
def query_annual_detail_sql(config:dict[str,Any],body:dict[str,Any],first:int,last:int) -> dict[str,Any]:
    ...
    if groups:item_filter+=f" AND CONVERT(varchar(40),P.IdAgrupamento) IN ({','.join('?' for _ in groups)})";item_values.extend(groups)
