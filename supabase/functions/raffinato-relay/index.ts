...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === "sales_bi_dashboard") {
      ...
      const revenue=rows.reduce((s,x)=>s+x.valor_atribuido,0),quantity=rows.reduce((s,x)=>s+x.quantidade_atribuida,0);return json({schema_version:1,totalizadores:{faturamento:revenue,vendas:eligibleIds.size,itens:quantity,ticket_medio:eligibleIds.size?revenue/eligibleIds.size:0},produtos:products,formas_pagamento:payments,canais:channels,dias_semana:days,horarios:hours,agrupamentos:groups,origem_consulta:"bi_agregado_remoto"});
    }
